...
def load_chunks() -> list[dict]:
    """Load recursive chunks aligned with the embedding matrix (5941 entries).
    """
    with open(data_dir / "chunks" / "recursive_chunks.json") as f:
        return json.load(f)

def load_embeddings() -> np.ndarray:
    """Load contextual embeddings (5941 x 768, float32).
    """
    ...
    for start in range(0, total, batch_size):
        ...
        for i in range(start, end):
            ...
            points.append(models.PointStruct(
                ...
                payload={k: c[k] for k in ("book_id", "book_title", "author", "chapter_number", "chapter_title", "chunk_index", "total_chunks_in_chapter", "text")}))
