...
For temporal queries, restricts traversal to PRECEDES edges (the ordering
relationship actually produced by the extraction pipeline; an earlier version
of this agent filtered on "OCCURS_BEFORE", which matches zero edges).
On retry, increases max_hops, broadens top_k, or drops the relationship filter.
"""
...
def run_graph_rag(state: dict[str, Any], sm_client: Supermemory,
                  chunks_index: dict | None = None) -> AgentResult:
    ...
    relationship_type: str | None = "PRECEDES" if understanding.query_type == "temporal" else None
    max_hops: int = 2
    top_k: int = 10
