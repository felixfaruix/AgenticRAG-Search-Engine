...
The taxonomy defines 8 entity types: LiteraryWork, Author, Character, Location, Object, Event, TextualComponent, and Theme. The ontology defines 12 relationship types: WROTE (Author → LiteraryWork), APPEARS_IN (Character → LiteraryWork), SET_IN (LiteraryWork → Location), RELATED_TO (Character → Character), PARTICIPATES_IN (Character → Event), PRECEDES (Event → Event), IS_PART_OF (TextualComponent → LiteraryWork), IS_SUBCOMPONENT_OF (TextualComponent → TextualComponent), OCCURS_IN (Event → TextualComponent), MANIFESTS_THEME (Event → Theme), USES (Character → Object), and TRAVELS_TO (Character → Location).

The schema is generated by an LLM using competency questions following the CQ-by-CQ methodology (Chevignard et al., ESWC 2025), where questions alone — not sample passages — are sufficient input for schema generation. The schema is validated against the corpus by extracting triples from random 800-word passages and measuring per-entity-type hit rates. Storage uses property graphs rather than RDF, because property graphs optimize for traversal speed and all retrieval-augmented generation tooling integrates natively with them.
