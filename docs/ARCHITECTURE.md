...
|---|---|---|
| Supermemory memory graph | Validated triples with source chunks, ontology edges, per-book containers | Graph RAG Agent |
| Supermemory review containers | Rejected triples with validation scores, per-book | Debugging, evaluation |
| Recursive vector index | 512-token chunks with book/chapter/position metadata, no enrichment | Vector RAG Agent (keyword search) |
| Contextual vector index | 512-token chunks with contextual headers and book/chapter/position metadata | Vector RAG Agent (semantic search) |
| Book-level vector index | ~500-word book summaries with embeddings | Thematic Agent |
| Supermemory agent containers | Per-agent session state, retry history, synthesis feedback | All agents |
...
Each agent gets two Supermemory containers per session. The scratchpad (`agent_{type}_{session}_scratchpad`) stores a human-readable log of every retrieval attempt: which tool was called, with what parameters, how many passages came back, and any grounding feedback that triggered a retry. The content is semantically indexed so the agent can search its own history before choosing an alternative strategy. The results container (`agent_{type}_{session}_results`) stores the query text and retrieved passage texts as content, with the full structured entry in metadata. This is what the orchestrator and other agents search when they need prior retrieval output. Container names are deterministic from `(agent_type, session_id)` and auto-created by Supermemory on first write. The session ID is generated once per query by the orchestrator.
...
3. Three vector indexes built from these chunks: recursive (no enrichment, for BM25), contextual (with model-generated headers, for dense search), book-level (one summary per book)
...
7. Validation agent scores each triple against its source text (Instructor, up to three retries)
8. Validated triples stored in Supermemory per-book containers; rejected triples stored in review containers

The chunk metadata enables filtered retrieval: when the orchestrator resolves entities to a specific book, the vector retrieval agent scopes BM25 or dense search to only chunks from that book rather than the full corpus. Chapter number and chunk position within chapter enable relative location of a passage in the narrative, which supports temporal reasoning even outside the graph retrieval path.

The recursive index serves BM25 keyword search with clean term frequencies: no injected context words that would dilute the inverse document frequency signal. The contextual index serves dense semantic search with resolved pronouns and references. Both indexes contain all books with metadata filtering, not separate indexes per book. Validation is offline; runtime agents operate on already-validated data.

Supermemory container layout:

| Container | Contents |
|---|---|
| `book_{key}` (one per book) | Validated triples with source chunks, ontology-aware edges |
| `review_{key}` (one per book) | Rejected triples with validation scores, audit trail |
| `agent_{type}_{session}_scratchpad` (per agent per session) | Attempt logs: tool name, parameters, outcome, grounding feedback. Searched by the agent itself on retry |
| `agent_{type}_{session}_results` (per agent per session) | Query text + retrieved passage texts. Searched by orchestrator and other agents |
...
On failure, the synthesis agent writes grounding feedback to the originating agent's scratchpad container via `write_scratchpad`. The agent searches its own scratchpad to see what it already tried, adjusts its retrieval strategy, and retries. On success (or after maximum two retries), the agent writes its final output to the results container via `write_results`. Maximum two retries (three total attempts). On final failure, the system returns the best available answer with a low-confidence flag.
...
Synthesis Loop:
  ...
  Agent retrieves passages
          |
          v
  Agent writes attempt to scratchpad
          |
          v
  ...
  Grounding check
     ...
     pass | fail
      |       |
      v       v
  Agent      Synthesis writes feedback
  writes     to agent's scratchpad
  results           |
      |              v
      v         Agent reads own scratchpad,
  Return        retries with adjusted strategy
  answer        (max 2 retries)
```

The grounding check, scratchpad write, and results write are tools available to the synthesis agent and retrieval agents respectively. The scratchpad creates a traceable retry history; the results container holds the clean output for cross-agent consumption.
